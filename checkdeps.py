...
from depends import detectOS, PACKAGES, PACKAGE_MANAGER
...
def prereqToPackages():
    ...
    print(
        "%s %s"
        % (
            PACKAGE_MANAGER[detectOS()],
            " ".join(PACKAGES[x][detectOS()] for x in detectPrereqs()),
        )
    )
...
def compilerToPackages():
    if not detectOS() in COMPILING:
        return
    print("%s %s" % (PACKAGE_MANAGER[detectOS.result], COMPILING[detectOS.result]))
...
if (not compiler or prereqs) and detectOS() in PACKAGE_MANAGER:
    print(
        ...
        "to install the missing dependencies." % detectOS()
    )
...
for lhs, rhs in EXTRAS_REQUIRE.items():
    if OPSYS is None:
        break
    if rhs and any(
        [EXTRAS_REQUIRE_DEPS[x][OPSYS] for x in rhs if x in EXTRAS_REQUIRE_DEPS]
    ):
        try:
            import_module(lhs)
        except Exception:
...
if detectOS() == "Ubuntu 20":
    print("Qt interface isn't supported in %s" % detectOS())
