...
from helper_sql import sql_ready, sql_timeout, SqlBulkExecute, sqlExecute, sqlQuery
from network import knownnodes, invQueue
from network.node import Peer
from tr import _translate

print("DEBUG: LOADED CLASS_OBJECTPROCESSOR WITH VERSION 4 FIX")
...
class objectProcessor(threading.Thread):
        ...
        while True:
            ...
            try:
                ...
                # the shutdown variable.
                    ...
                    if isinstance(objectType, int):
                        logger.info(
                            "Don't know how to handle object type 0x%08X", objectType
                        )
                    else:
                        logger.info(
                            "Don't know how to handle object type %s", objectType
                        )
            ...
            if state.shutdown:
                ...
                with SqlBulkExecute() as sql:
                    while not queues.objectProcessorQueue.empty():
                        objectType, data = queues.objectProcessorQueue.get()
                        sql.execute(
            ...
            queues.UISignalQueue.put(
                (
                    ...
                    (
                        bytes(data[readPosition:]),
                        _translate(
                            "MainWindow", "Acknowledgement of the message received %1"
    ...
    def processpubkey(self, data):
        ...
        if addressVersion == 4:
            ...
            toAddress = state.neededPubkeys[bytes(tag)][0]
    ...
    def processmsg(self, data):
        ...
        if (
            ...
            logger.error("DEBUG: processmsg Pubkey insert FAILED for %s", fromAddress)
        else:
            logger.debug("processmsg Pubkey insert SUCCESS for %s", fromAddress)
        ...
        # my chan addresses:
        if decodeAddress(toAddress)[1] >= 3 and not config.safeGetBoolean(
            toAddress, "chan"
        ):
        ...
        if not blockMessage:
            ...
            if (
                config.safeGetBoolean(toAddress, "mailinglist")
        ...
        if (
            self.ackDataHasAValidHeader(ackData)
    ...
    def processbroadcast(self, data):
        ...
        if broadcastVersion == 4:
            ...
            signedData = bytes(data[8:readPosition])
            cryptorObject = shared.MyECSubscriptionCryptorObjects[bytes(embeddedTag)]
            try:
                decryptedData = cryptorObject.decrypt(
                    data[readPosition:], hmac_prefix=data[:readPosition]
                )
                ...
                return logger.debug(
                    "Broadcast version %s decryption Unsuccessful.", broadcastVersion
                )
        ...
        logger.debug(
            "Address version detection: version=%d, length=%d",
            sendersAddressVersion,
            sendersAddressVersionLength,
        )
        logger.debug(
            "Address version details: version type=%s, broadcast version=%d",
            type(sendersAddressVersion).__name__,
            broadcastVersion,
        )

        # DEBUG: Trace the entire address decoding process
        logger.debug(
            "Address decoding process: starting with version=%d", sendersAddressVersion
        )

        if broadcastVersion == 4:
            if sendersAddressVersion < 2 or sendersAddressVersion > 4:
                print(
                    f"DEBUG_BROADCAST: Unsupported version {sendersAddressVersion} in v4 "
                    f"broadcast {hexlify(inventoryHash)}. Target RIPE: {hexlify(toRipe)}. "
                    f"Time: {int.from_bytes(data[8:16], 'big')}"
                )
                return logger.warning(
                    "Cannot decode senderAddressVersion other than 2, 3, or 4."
            ...
            if sendersAddressVersion < 4:
                print(
                    f"DEBUG_BROADCAST: Unsupported version {sendersAddressVersion} in v5 "
                    f"broadcast {hexlify(inventoryHash)}. Target RIPE: {hexlify(toRipe)}. "
                    f"Time: {int.from_bytes(data[8:16], 'big')}"
                )
                return logger.info(
        ...
        if (
            ...
            logger.error(
                "DEBUG: processbroadcast Pubkey insert FAILED for %s", fromAddress
            )
        else:
            logger.debug("processbroadcast Pubkey insert SUCCESS for %s", fromAddress)
            # Check to see whether we happen to be awaiting this
            # pubkey in order to send a message. If we are, it will do the POW
            # and send it.
            self.possibleNewPubkey(fromAddress)
    ...
    def possibleNewPubkey(self, address):
        ...
        """
        # import traceback
        # logger.critical("DEBUG: possibleNewPubkey called for %s", address)
        # logger.critical("Traceback: %s", ''.join(traceback.format_stack()))
        ...
        if addressVersion <= 3:
            if address in state.neededPubkeys:
                ...
                logger.debug(
                    "We don't need this pub key. We didn't ask for it. For address: %s",
                    address,
                )
