...
class TestAPIShutdown(TestAPIProto):
    ...
    def test_shutdown(self):
        ...
        try:
            ...
            self.fail("%s has not stopped in 20 sec" % " ".join(self._process_cmd))
...
class TestAPI(TestAPIProto):
    ...
    def _add_random_address(self, label):
        addr = self.api.createRandomAddress(base64.encodebytes(label))
        return addr

    def test_user_password(self):
        """Trying to connect with wrong username/password"""
        api_wrong = xmlrpc_client.ServerProxy("http://test:wrong@127.0.0.1:8442/")
        with self.assertRaises(xmlrpc_client.ProtocolError):
            api_wrong.clientStatus()

    def test_connection(self):
        """API command 'helloWorld'"""
        self.assertEqual(self.api.helloWorld("hello", "world"), "hello-world")
    ...
    def test_invalid_method(self):
        ...
        self.assertEqual(self.api.test(), "API Error 0020: Invalid method: test")

    def test_message_inbox(self):
        """Test message inbox methods"""
        self.assertEqual(
            len(json.loads(self.api.getAllInboxMessages())["inboxMessages"]),
            4,
            # Custom AssertError message for details
            json.loads(self.api.getAllInboxMessages())["inboxMessages"],
        )
        self.assertEqual(
            len(json.loads(self.api.getAllInboxMessageIds())["inboxMessageIds"]), 4
        )
        self.assertEqual(
            len(
                json.loads(
                    self.api.getInboxMessageById(hexlify(sample_inbox_msg_ids[2]))
                )["inboxMessage"]
            ),
            1,
        )
        self.assertEqual(
            len(
                json.loads(
                    self.api.getInboxMessagesByReceiver(sample_deterministic_addr4)
                )["inboxMessages"]
            ),
            4,
        )
    ...
    def test_message_trash(self):
        ...
        for msgid in sample_inbox_msg_ids[:2]:
            self.assertEqual(
                self.api.undeleteMessage(hexlify(msgid)), "Undeleted message"
            )
        self.assertEqual(
            len(json.loads(self.api.getAllInboxMessageIds())["inboxMessageIds"]),
    ...
    def test_list_addresses(self):
        ...
        self.assertEqual(
            json.loads(self.api.listAddresses()).get("addresses"), self.addresses
        )

    def test_decode_address(self):
        """Checking the return of API command 'decodeAddress'"""
        result = json.loads(self.api.decodeAddress(sample_deterministic_addr4))
    ...
    def test_create_deterministic_addresses(self):
        ...
        self.assertRegex(
            self.api.getDeterministicAddress(self._seed, 3, 2), r"API Error 0003:"
        )
        self.assertRegex(
        ...
        self.assertRegex(
            self.api.createDeterministicAddresses("", 1), r"API Error 0001:"
        )
        self.assertRegex(
        ...
        addresses = json.loads(self.api.createDeterministicAddresses(self._seed, 2, 4))[
            "addresses"
        ]
    ...
    def test_addressbook(self):
        ...
        self.api.addAddressBookEntry(
            sample_deterministic_addr4, base64.encodebytes(b"tiger_4")
        )
        # Check addressbook entry
        entries = json.loads(self.api.listAddressBookEntries()).get("addresses")[0]
        self.assertEqual(entries["address"], sample_deterministic_addr4)
        self.assertEqual(base64.decodestring(entries["label"]), "tiger_4")
    ...
    def test_blackwhitelist(self):
        ...
        self.api.addBlacklistEntry(
            sample_deterministic_addr4, base64.encodestring("tiger_4")
        )
        # Check list entry
        ...
        self.assertRegex(self.api.setBlackWhitelistKind("yellow"), r"^API Error 0028:")
        # Change kind
    ...
    def test_send(self):
        ...
        try:
            ...
            for m in json.loads(self.api.getSentMessagesByAddress(addr))[
                "sentMessages"
            ]:
            ...
            self.assertEqual(
                json.loads(self.api.getSentMessageByAckData(ackdata))["sentMessage"][0][
                    "message"
                ],
                sent_msg,
            )
            ...
            self.assertIsNone(json.loads(self.api.getSentMessageByAckData(ackdata)))
        finally:
            self.assertEqual(self.api.deleteAddress(addr), "success")
    ...
    def test_send_broadcast(self):
        ...
        ackdata = self.api.sendBroadcast(addr, base64.encodestring("test_subject"), msg)
        ...
        try:
            ...
            if status == "notfound":
                raise KeyError
            self.assertIn(
                status, ("doingbroadcastpow", "broadcastqueued", "broadcastsent")
            )
            ...
            try:
                ...
                self.fail("Could not find sent message in sent messages")
            self.assertEqual(
                json.loads(self.api.getSentMessageById(sent_msgid))["sentMessage"][0][
                    "message"
                ],
                sent_msg,
            )
            self.assertIn(
                {"msgid": sent_msgid},
                json.loads(self.api.getAllSentMessageIds())["sentMessageIds"],
            )
            ...
            self.assertEqual(self.api.deleteAndVacuum(), "done")
            self.assertIsNone(json.loads(self.api.getSentMessageById(sent_msgid)))
            ...
            self.assertRegex(result, r"^API Error 0014:")
        finally:
            self.assertEqual(self.api.deleteAddress(addr), "success")
        ...
        self.assertRegex(result, r"^API Error 0013:")

    def test_chan(self):
        """Testing chan creation/joining"""
        # Create chan with known address
        self.assertEqual(self.api.createChan(self._seed), sample_deterministic_addr4)
        # cleanup
        self.assertEqual(self.api.leaveChan(sample_deterministic_addr4), "success")
