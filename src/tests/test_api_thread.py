...
class TestAPIThread(TestPartialRun):
    ...
    def test_statusbar(self):
        ...
        self.assertEqual(self.api.statusBar(sample_statusbar_msg), "success")
        try:
    ...
    def test_disseminate_preencrypted(self):
        ...
        self.assertEqual(
            unhexlify(
                self.api.disseminatePreparedObject(hexlify(sample_object_data).decode())
            ),
            calculateInventoryHash(sample_object_data),
        )
        update_object = (
            b"\x00" * 8 + pack(">Q", int(time.time() + 7200)) + sample_object_data[16:]
        )
        invhash = unhexlify(
            self.api.disseminatePreEncryptedMsg(hexlify(update_object).decode())
        )
