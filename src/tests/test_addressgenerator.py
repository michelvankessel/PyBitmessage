...
class TestAddressGenerator(TestPartialRun):
    ...
    def setUpClass(cls):
        ...
        # Clean up any lingering test addresses from previous runs
        for addr in [sample_deterministic_addr3, sample_deterministic_addr4]:
            if cls.config.has_section(addr):
                cls.config.remove_section(addr)
        cls.config.save()
    ...
    @unittest.skipIf(sys.hexversion < 0x3000000, "assertLogs is new in version 3.4")
    ...
    def test_deterministic(self):
        ...
        self.command_queue.put(
            ("getDeterministicAddress", 3, 1, TEST_LABEL, 1, sample_seed, False)
        )
        ...
        self.assertEqual(
            sample_deterministic_addr3,
            self._execute(
                "createDeterministicAddresses", 3, 1, TEST_LABEL, 2, sample_seed, False
            ),
        )
        ...
        self.assertTrue(self.config.getboolean(sample_deterministic_addr4, "chan"))
        self.assertTrue(self.config.getboolean(sample_deterministic_addr4, "enabled"))
        ...
        self.assertEqual(
            self._execute(
                "joinChan", sample_deterministic_addr3, TEST_LABEL, sample_seed, False
            ),
            sample_deterministic_addr3,
        )
    ...
    def test_random(self):
        ...
        self.assertEqual(self.worker_queue.get(), ("sendOutOrStoreMyV4Pubkey", addr))
