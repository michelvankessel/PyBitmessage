...
knownNodes: dict[int, dict[Peer, dict]] = {stream: {} for stream in range(1, 4)}
...
DEFAULT_NODES = (
    # Updated 2026-01-08 from bootstrap8444.bitmessage.org
    Peer("185.158.248.216", 8444),
    Peer("85.114.135.102", 8444),
    Peer("85.25.152.9", 8444),
    Peer("194.164.163.84", 8444),
    # From bootstrap8080.bitmessage.org
    Peer("185.19.31.46", 8080),
    Peer("158.69.63.42", 8080),
)
        ...
        for stream in knownNodes.keys():
            try:
                knownNodes[stream][peer]["rating"] = min(
                    knownNodes[stream][peer]["rating"] + increaseAmount, maxRating
                )
            except KeyError:
                pass
        ...
        for stream in knownNodes.keys():
            try:
                knownNodes[stream][peer]["rating"] = max(
                    knownNodes[stream][peer]["rating"] - decreaseAmount, minRating
                )
            except KeyError:
                pass
...
def trimKnownNodes(recAddrStream=1):
    ...
    if len(knownNodes[recAddrStream]) < config.safeGetInt("knownnodes", "maxnodes"):
        return
    with knownNodesLock:
        oldestList = sorted(knownNodes[recAddrStream], key=lambda x: x["lastseen"])[
...
def dns():
    ...
    for port in [8080, 8444]:
        addKnownNode(1, Peer("bootstrap%s.bitmessage.org" % port, port))
        ...
        for stream in knownNodes:
            ...
            for node in keys:
                ...
                try:
                    ...
                    if (
                        age > 10800
                        and knownNodes[stream][node]["rating"] <= knownNodesForgetRating
                    ):
                        needToWriteKnownNodesToDisk = True
                        del knownNodes[stream][node]
                        continue
