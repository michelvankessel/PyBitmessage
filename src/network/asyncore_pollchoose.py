...
WSAEWOULDBLOCK = getattr(errno, 'WSAEWOULDBLOCK', EWOULDBLOCK)
WSAENOTSOCK = getattr(errno, 'WSAENOTSOCK', ENOTSOCK)
WSAECONNRESET = getattr(errno, 'WSAECONNRESET', ECONNRESET)
WSAEADDRINUSE = getattr(errno, 'WSAEADDRINUSE', EADDRINUSE)
...
def write(obj):
    ...
    if not can_send():
        return
    # print(f"DEBUG: ASYNCORE WRITE DISPATCH: {obj}")
    try:
