...
class BMProto(AdvancedDispatcher, ObjectTracker):
    ...
    def state_bm_header(self):
        ...
        self.command = self.command.rstrip(b"\x00").decode("ascii")
        ...
        if self.payloadLength > protocol.MAX_MESSAGE_SIZE:
            self.invalid = True
        self.set_state(
            "bm_command", length=protocol.Header.size, expectBytes=self.payloadLength
        )
        return True
    ...
    def state_bm_command(self):
        ...
        if not self.invalid:
            try:
                # print(
                #    f">>> COMMAND RECEIVED: {self.destination.host}:{self.destination.port} cmd={self.command} len={self.payloadLength}"
                # )
                retval = getattr(self, "bm_command_" + self.command.lower())()
    ...
    def decode_payload_string(self, length):
        ...
        value = bytes(self.payload[self.payloadOffset:self.payloadOffset + length])
    ...
    def decode_payload_node(self):
        ...
        if host[0:12] == b"\x00\x00\x00\x00\x00\x00\x00\x00\x00\x00\xff\xff":
            ...
            host = base64.b32encode(host[6:]).decode("ascii").lower() + ".onion"
        else:
    ...
    def decode_payload_content(self, pattern="v"):
        ...
        def decode_simple(self, char="v"):
            ...
            if char == "H":
                ...
                return struct.unpack(
                    ">H", self.payload[self.payloadOffset - 2:self.payloadOffset]
                )[0]
            if char == "I":
                self.payloadOffset += 4
                return struct.unpack(
                    ">I", self.payload[self.payloadOffset - 4:self.payloadOffset]
                )[0]
            if char == "Q":
                self.payloadOffset += 8
                return struct.unpack(
                    ">Q", self.payload[self.payloadOffset - 8:self.payloadOffset]
                )[0]
            return None
        ...
        while True:
            ...
            if i in "0123456789" and (
                size is None or parserStack[-1][3][parserStack[-1][4] - 1] not in "lL"
            ):
                try:
                    size = size * 10 + int(i)
                except TypeError:
                ...
                isArray = i == "L"
                logger.debug("Pattern %s: size=%d, isArray=%s", i, size, isArray)
                ...
                if isArray:
                    ...
                    parserStack.append(
                        [
                            ...
                            size,
                            isArray,
                            parserStack[-1][3][parserStack[-1][4]:j + 1],
                            0,
                            [],
                        ]
                    )
                ...
                parserStack[-1][5].append(
                    decode_simple(self, parserStack[-1][3][parserStack[-1][4]])
                )
                size = None
            else:
                size = None
                ...
                if parserStack[depth][4] >= len(parserStack[depth][3]):
                    ...
                    if parserStack[depth][1] <= 0:
                        if depth == 0:
                            ...
                            result = parserStack[depth][5]
                            logger.debug("decode_payload_content returning: %s", result)
                            return result
                        del parserStack[-1]
                        continue
                    break
        ...
        if self.payloadOffset > self.payloadLength:
            logger.debug(
                "Insufficient data %i/%i", self.payloadOffset, self.payloadLength
            )
    ...
    def bm_command_error(self):
        ...
        # print(
        #    f">>> ERROR RECEIVED from {self.destination.host}:{self.destination.port}: fatal={fatalStatus}, text={errorText}"
        # )

        # Aggressively penalize nodes that reject our connections
        # This helps avoid repeatedly hitting nodes that refuse us
        if fatalStatus == 2:  # fatal error
            error_lower = errorText.decode("utf-8", "ignore").lower()
            if any(
                ...
                # Decrease rating multiple times to quickly avoid this node
                for _ in range(5):
                    knownnodes.decreaseRating(self.destination)
                logger.info(
                    "Aggressively decreased rating for %s due to rejection",
                    self.destination.host,
                )
    ...
    def _command_inv(self, extend_dandelion_stem=False):
        ...
        logger.debug("decode_payload_content('l32s') returned %d items", len(items))
        for i, item in enumerate(items[:5]):
            logger.debug(
                "Item %d: type=%s, len=%s, value=%s",
                i,
                type(item).__name__,
                len(item) if hasattr(item, "__len__") else "N/A",
                repr(item)[:50],
            )
        ...
        for item in items:
            if isinstance(item, (bytes, bytearray)) and len(item) == 32:
                ...
                logger.warning(
                    "Invalid inventory hash in inv message: type=%s, len=%s, value=%s",
                    type(item).__name__,
                    len(item) if hasattr(item, "__len__") else "N/A",
                    repr(item)[:50],
                )
    ...
    def bm_command_object(self):
        ...
        state.Inventory[self.object.inventoryHash] = (
            ...
            self.object.streamNumber,
            memoryview(self.payload[objectOffset:]),
            self.object.expiresTime,
            memoryview(self.object.tag),
        )
        self.handleReceivedObject(self.object.streamNumber, self.object.inventoryHash)
        invQueue.put(
            (self.object.streamNumber, self.object.inventoryHash, self.destination)
        )
        return True
    ...
    def bm_command_portcheck(self):
        ...
        if self.isOutbound or self.portCheckRequested:
            return True
        self.portCheckRequested = True
        portCheckerQueue.put(Peer(self.destination.host, self.peerNode.port))
        return True
    ...
    def bm_command_version(self):
        ...
        self.userAgent = userAgent.decode("utf-8", "ignore")
        ...
        ua_valid = re.match(r"^/[a-zA-Z]+:[0-9]+\.?[\w\s\(\)\./:;-]*/$", self.userAgent)
        if not ua_valid:
        ...
        if not self.isOutbound:
            ...
            logger.debug(
                "%(host)s:%(port)i sending version", self.destination._asdict()
            )
        if (
            self.services & protocol.NODE_SSL == protocol.NODE_SSL
        ) and protocol.haveSSL(not self.isOutbound):
            self.isSSL = True
        logger.info(
            "%s:%i: VERSION exchanged - remote=%s, userAgent=%s, isSSL=%s, verackSent=%s, verackReceived=%s",
            self.destination.host,
            self.destination.port,
            self.remoteProtocolVersion,
            self.userAgent,
        ...
        if not self.verackReceived:
            return True
        # Only transition if we have received the version message too
        if self.remoteProtocolVersion:
            self.set_state(
                "tls_init" if self.isSSL else "connection_fully_established",
                length=self.payloadLength,
                expectBytes=0,
    ...
    def peerValidityChecks(self):
        ...
        if not self.streams:
            ...
            return False
        # NOTE: Removed flawed check that rejected all second connections from
        # the same IP. The checkSocksIP logic was inverted, causing legitimate
        # connections to be rejected. The "server full" check below properly
        # handles duplicate connections by checking specific IP+port pairs.
        if not self.isOutbound:
            # incoming from a peer we're connected to as outbound,
            # or server full report the same error to counter deanonymisation
            if Peer(
                self.destination.host, self.peerNode.port
            ) in connectionpool.pool.inboundConnections or len(
                connectionpool.pool
            ) > config.safeGetInt(
                "bitmessagesettings", "maxtotalconnections"
            ) + config.safeGetInt("bitmessagesettings", "maxbootstrapconnections"):
                self.append_write_buf(
                    protocol.assembleErrorMessage(
        ...
        if connectionpool.pool.isAlreadyConnected(self.nonce):
            self.append_write_buf(
                protocol.assembleErrorMessage(
                    errorText="I'm connected to myself. Closing connection.", fatal=2
                )
            )
            logger.debug(
    ...
    def handle_close(self):
        ...
        try:
            ...
            try:
                logger.debug(
                    "%s:%i: closing", self.destination.host, self.destination.port
                )
            except AttributeError:
