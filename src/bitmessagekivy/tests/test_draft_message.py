...
class DraftMessage(TeleniumTestProcess):
    ...
    def test_draft_screen(self):
        ...
        self.assert_wait_no_except(
            "//ScreenManager[@current]", timeout=15, value="inbox"
        )
    ...
    def test_save_message_to_draft(self):
        ...
        self.assert_wait_no_except(
            "//ScreenManager[@current]", timeout=10, value="inbox"
        )
        # Click on Composer Icon(Plus icon)
        self.cli.wait_click(
            '//ComposerButton[0]/MDFloatingActionButton[@icon="plus"]', timeout=5
        )
        # Checking Message Composer Screen(Create)
        self.assertExists('//ScreenManager[@current="create"]', timeout=5)
        # ADD SUBJECT
        self.cli.setattr(
            "//DropDownWidget/ScrollView[0]//MyMDTextField[0]",
            "text",
            self.test_subject,
        )
        # Checking Subject Field is Entered
        self.assertExists(
            f'//DropDownWidget/ScrollView[0]//MyMDTextField[@text="{self.test_subject}"]',
            timeout=5,
        )
        # ADD MESSAGE BODY
        self.cli.setattr(
        ...
        self.assertExists(
            f'//DropDownWidget/ScrollView[0]/BoxLayout[0]/ScrollView[0]/MDTextField[@text="{self.test_body}"]',
            timeout=5,
        )
        # Click on "Send" Icon
        ...
        self.assertExists(
            '//DropDownWidget/ScrollView[0]//MyTextInput[@text=""]', timeout=5
        )
        # Entering Receiver Address
        self.cli.setattr(
            "//DropDownWidget/ScrollView[0]//MyTextInput[0]",
            "text",
            test_address["auto_responder"],
        )
        # Checking Receiver Address filled or not
        self.assertExists(
            f'//DropDownWidget/ScrollView[0]//MyTextInput[@text="{test_address["auto_responder"]}"]',
            timeout=5,
        )
        # Checking the sender's Field is empty
        self.assertExists(
            '//DropDownWidget/ScrollView[0]//BoxLayout[1]/MDTextField[@text=""]',
            timeout=5,
        )
        # Assert to check Sender's address dropdown open or not
        self.assertEqual(
            self.cli.getattr("//Create//CustomSpinner[@is_open]", "is_open"), False
        )
        ...
        sender_address = self.cli.getattr(
            "//DropDownWidget/ScrollView[0]//BoxLayout[1]/MDTextField[@text]", "text"
        )
        self.assertExists(
            f'//DropDownWidget/ScrollView[0]//BoxLayout[1]/MDTextField[@text="{sender_address}"]',
            timeout=5,
        )
        # CLICK BACK-BUTTON
        self.cli.wait_click(
            '//MDToolbar/BoxLayout[0]/MDActionTopAppBarButton[@icon="arrow-left"]',
            timeout=5,
        )
    ...
    def test_edit_and_resend_draft_messgae(self):
        ...
        self.assertEqual(
            len(self.cli.select("//SwipeToDeleteItem[0]//TwoLineAvatarIconListItem")), 1
        )
        ...
        self.cli.wait_click(
            "//SwipeToDeleteItem[0]//TwoLineAvatarIconListItem[0]", timeout=5
        )
        ...
        self.cli.wait_click(
            '//MDToolbar/BoxLayout[2]/MDActionTopAppBarButton[@icon="pencil"]',
            timeout=5,
        )
        ...
        self.assertExists(
            f'//DropDownWidget/ScrollView[0]//ScrollView[0]/MDTextField[@text="{self.test_body}"]',
            timeout=5,
        )
    ...
    def test_delete_draft_message(self):
        ...
        self.cli.wait_drag(
            "//MDList[0]//AvatarSampleWidget",
            "//MDList[0]//TimeTagRightSampleWidget",
            2,
            timeout=5,
        )
        ...
        self.assertNotExists(
            "//SwipeToDeleteItem[0]//TwoLineAvatarIconListItem", timeout=5
        )
        # Message count should be zero
        self.assertEqual(
            len(
                self.cli.select("//SwipeToDeleteItem[0]//TwoLineAvatarIconListItem[0]")
            ),
            0,
        )
