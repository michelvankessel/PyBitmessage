...
try:
    Dialog = __import__("dialog").Dialog
except ImportError:
    Dialog = None
import helper_sent
import l10n
...
# Python 3.13 only - unicode is always str
...
def drawtab(stdscr):
    ...
    if menutab in range(1, len(menu) + 1):
        if menutab == 1:  # Inbox
            ...
            for i, item in enumerate(
                subscriptions[
                    max(min(len(subscriptions) - curses.LINES + 6, subcur - 5), 0):
                ]
            ):
                if 6 + i < curses.LINES:
                    a = 0
            ...
            for i, item in enumerate(
                blacklist[
                    max(min(len(blacklist) - curses.LINES + 6, blackcur - 5), 0):
                ]
            ):
                if 7 + i < curses.LINES:
                    a = 0
            ...
            stdscr.addstr(
                ...
                40,
                "Processed "
                + str(state.numberOfPubkeysProcessed).ljust(4)
                + " public keys.",
            )
            ...
            if n > 0:
                ...
                logpad.refresh(
                    n - curses.LINES + 2, 0, 14, 6, curses.LINES - 2, curses.COLS - 7
                )
    stdscr.refresh()
                                ...
                                if t == "1":
                                    ...
                                    if r == d.DIALOG_OK:
                                        if t[0] == t[1]:
                                            ...
                                            scrollbox(
                                                d, str("Passphrases do not match")
                                            )
            ...
            if addr != "":
                ...
                if status != "success":
                    ...
                    if status == "missingbm":
                        ...
                        err += (
                            "The address version is too high. Either you need to upgrade your Bitmessage software"
                            " or your acquaintance is doing something clever."
                        )
                    elif status == "ripetooshort":
                        err += (
                            "Some data encoded in the address is too short. There might be something wrong with"
                            " the software of your acquaintance."
                        )
                    elif status == "ripetoolong":
                        err += (
                            "Some data encoded in the address is too long. There might be something wrong with"
                            " the software of your acquaintance."
                        )
                    elif status == "varintmalformed":
                        err += (
                            "Some data encoded in the address is malformed. There might be something wrong with"
                            " the software of your acquaintance."
                        )
                    else:
...
def run(stdscr):
    ...
    for addressInKeysFile in configSections:
        ...
        addresses.append(
            [config.get(addressInKeysFile, "label"), isEnabled, addressInKeysFile]
        )
        # Set address color
        if not isEnabled:
