...
class TestSettings(TestBase):
    ...
    def test_udp(self):
        ...
        self.dialog.accept()
        self.assertEqual(
            not udp_setting, config.safeGetBoolean("bitmessagesettings", "udp")
        )
        time.sleep(5)
        for thread in threading.enumerate():
            if thread.name == "Announcer":  # find Announcer thread
                if udp_setting:
                    self.fail("Announcer thread is running while udp set to False")
                break
        else:
            if not udp_setting:
    ...
    def test_styling(self):
        ...
        self.assertEqual(style_control.currentText(), self.app.get_windowstyle())
        ...
        def call_font_dialog():
            ...
            self.assertEqual(
                config.safeGet("bitmessagesettings", "font"),
                f"{selected_font.family()},{selected_font.pointSize()}",
            )
