...
def loadConfig():
    ...
    if state.appdata:
        ...
        needToCreateKeysFile = (
            config.safeGet("bitmessagesettings", "settingsversion") is None
        )
        if not needToCreateKeysFile:
            logger.info(
                "Loading config files from directory specified on startup: %s",
                state.appdata,
            )
    else:
        ...
        if config.safeGet("bitmessagesettings", "settingsversion"):
            ...
            if not needToCreateKeysFile:
                logger.info("Loading existing config files from %s", state.appdata)
    ...
    if needToCreateKeysFile:
        ...
        if StoreConfigFilesInSameDirectoryAsProgramByDefault:
            ...
            # the appdata folder
            state.appdata = ""
            logger.info("Creating new config files in same directory as program.")
        else:
...
def updateConfig():
    ...
    if settingsversion == 5:
        config.set("bitmessagesettings", "maxacceptablenoncetrialsperbyte", "0")
        config.set("bitmessagesettings", "maxacceptablepayloadlengthextrabytes", "0")
        settingsversion = 7
    ...
    if settingsversion == 8:
        ...
        previousTotalDifficulty = (
            int(config.getint("bitmessagesettings", "maxacceptablenoncetrialsperbyte"))
            / 320
        )
    ...
    if settingsversion == 9:
        for addressInKeysFile in config.addresses():
            try:
                previousTotalDifficulty = (
                    float(config.getint(addressInKeysFile, "noncetrialsperbyte")) / 320
                )
                previousSmallMessageDifficulty = (
                    float(config.getint(addressInKeysFile, "payloadlengthextrabytes"))
                    / 14000
                )
    ...
    # sanity check
    if config.safeGetInt("bitmessagesettings", "maxacceptablenoncetrialsperbyte") == 0:
        config.set(
            "bitmessagesettings",
            "maxacceptablenoncetrialsperbyte",
            str(
    ...
    if (
        config.safeGetInt("bitmessagesettings", "maxacceptablepayloadlengthextrabytes")
        == 0
    ):
        config.set(
            "bitmessagesettings",
    ...
    if not config.has_option("bitmessagesettings", "hidetrayconnectionnotifications"):
...
def adjustHalfOpenConnectionsLimit():
    ...
    if config.safeGet("bitmessagesettings", "socksproxytype", "none") != "none":
...
def start_proxyconfig():
    ...
    if proxy_type and proxy_type not in ("none", "SOCKS4a", "SOCKS5"):
        try:
            ...
            logger.error(
                "Failed to run proxy config plugin %s", proxy_type, exc_info=True
            )
